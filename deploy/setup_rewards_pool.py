...
def main():
    ...
    parser.add_argument(
        "--force", "-f",
        action="store_true",
        help="Delete existing key and import a new one"
    )
    parser.add_argument(
        "--config", "-c",
        action="store_true",
        help="Configure backend.env for existing key (enable quests/payouts)"
    )
    ...
    if key_exists(KEY_NAME):
        ...
        if not args.force:
            ...
            print()
            print(BOX_TOP)
            print(f"│ {GREEN}✅{RESET} EXISTING KEY - NO CHANGES MADE                                │")
            print("│                                                                    │")
            print(f"│ Address: {address:<57} │")
            if balance is not None:
                balance_str = format_mirage(balance)
                print(f"│ Balance: {balance_str:<57} │")
            print("│                                                                    │")
            print("│ Use --force to replace with a different seed phrase.               │")
            print(BOX_BOT)
            print()
            return 0
    ...
    if balance is not None:
        ...
        if mirage_balance < MIN_MIRAGE_BALANCE:
            ...
            print()
            print(BOX_TOP)
            print(f"│ {YELLOW}⚠️{RESET} LOW BALANCE WARNING                                          │")
            print("│                                                                    │")
            print(f"│ Current balance: {format_mirage(balance):<49} │")
            print(f"│ Minimum recommended: {min_str:<45} │")
            print("│                                                                    │")
            print("│ The wallet needs to be funded before rewards can be distributed.   │")
            print("│ You can continue setup now and fund later, or fund first and run   │")
