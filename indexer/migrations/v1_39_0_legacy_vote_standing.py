...
``user_community_stats`` keys on ``COALESCE(NULLIF(root_community,''), community)``,
...
def run(db, chain, logger):
    def _migrate(cur):
        ...
        communities = [t for t in (communities or [])]
        ...
        return f"recomputed {len(owners)} owners across {len(communities)} communities from {rekeyed} re-keyed comments"
