...
**Categories run in parallel by default.** Each concurrently running category leases its own wallet set out of a pool, so no two categories share `sub1`. Pool size is `--wallet-sets` or `MIRAGE_TEST_WALLET_SETS`, default 4. Use `--serial` to run every category alone on one set when reproducing a parallel failure.

**The faucet is consumable, so the suite returns what it did not spend.** A wallet set costs about 2.3M MIRAGE up front and the validator never gets it back on its own, so N sets used to drain the faucet N times faster until setup aborted with "faucet source balance too low". Teardown now sweeps the paid wallets back to the validator, which recovers roughly 80% and leaves a residual drain of about 0.5M per set per run (the subscription fees, which are genuinely spent). Setup still fails hard when the faucet cannot fund the requested pool; lower `--wallet-sets` or re-init with `scripts/reset_local_testnet.py`. Most of the up-front cost is `agent1` and `agent2` being funded for the retired Agent tier (500k and 1.5M) when they now only buy a level-1 subscription — right-sizing those would cut the per-set cost about fourfold and is worth doing.

A category belongs in `EXCLUSIVE_CATEGORIES` only when it contends on something a private wallet set cannot isolate — the Caddy rate limiter, chain-wide difficulty, the global anonymous feed, governance params, shared outbox rows, or a service restart. Write the reason next to the entry. Everything else is wallet-bound and runs concurrently; `WALLETLESS_CATEGORIES` takes no lease at all and `WALLETS` raises if such a category touches a wallet. A thread a test spawns itself must be started through `tests.common.parallel_map()`, or it inherits neither the wallet lease nor the category, and an unattributed skip escapes the release gate.
