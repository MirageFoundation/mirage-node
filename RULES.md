...
### Query Sources - Indexer-Only Backend Architecture

- **Backend reads ALL chain state from the indexer PostgreSQL database** (profiles, balances, params, difficulty, block hashes, supply, validators). No gRPC.
...
- **Indexer handles chain indexing only** — gRPC for queries, CometBFT WebSocket for live blocks, writes indexed state to PostgreSQL. Does NOT broadcast transactions.
- NO FALLBACKS. If the indexer DB is unavailable, the backend returns 503. Hard fail only.
