...
**Goal:** Register a new `default` theme that is selectable from Settings, renders every route without crashing, and uses visual tokens ported from `mirage-mobile-app`. No final visuals yet — this plan just establishes the foundation.
...
### In scope
- Create `web/frontend/src/themes/default/` with a complete, valid manifest.
...
Copy the full `web/frontend/src/themes/oldreddit/` folder to `web/frontend/src/themes/default/`. Then:

- Rename the default-exported manifest variable to `defaultManifest`.
- Change `id: 'oldreddit'` → `id: 'default'`.
- Change `label` and `description` to something like:
  - `label: 'Mirage App'`
  - `description: 'Mobile-app visuals on a Reddit-style desktop layout'`
- Update every `themeId` string inside `tokens.js` (`dark` / `light`) to `'default'`.
- Rename the shell file (for example `DefaultShell.js`) and update the import in `index.js`.
...
### Create
```
web/frontend/src/themes/default/
  index.js
  tokens.js
  Style.js
  DefaultShell.js
...
### Colors to map (dark + light) in `themes/default/tokens.js`
...
- [ ] `web/frontend/src/themes/manifests.js` imports and registers `defaultManifest`.
- [ ] The theme appears in the Settings theme picker (existing code iterates `THEMES`).
- [ ] Switching to `default` renders every route without the registry throwing (required component keys present, all routes implemented).
...
- **Cross-theme imports slipping in** → grep inside `themes/default` for `themes/oldreddit` and remove any leftover references.

---

## PR description template

> Adds a new `default` web theme scaffold cloned from `oldreddit` with tokens ported from `mirage-mobile-app`. Registered in the theme manifest but not set as default. No visual layout changes yet — follow-up PRs will replace shell, nav, feed, and routes.
