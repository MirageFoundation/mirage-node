...
A single focused sweep across the whole `default` theme to catch drift, verify every route behaves correctly in dark/light and at every breakpoint, and (optionally, in a follow-up PR) flip it to the default.
...
- [ ] Perform a vote, comment, follow, block, and report while on `default`.
...
- [ ] `grep -r "themes/oldreddit" web/frontend/src/themes/default/` returns 0 results.
- [ ] `grep -r "themes/bluemoon" web/frontend/src/themes/default/` returns 0 results.
...
1. In `web/frontend/src/themes/manifests.js`, move `defaultManifest` to position 0 in `THEME_MANIFESTS`.
...
> Final polish sweep + QA on the `default` theme. No behavior changes. Closes sub-plan 06.9 and Plan 06 entirely. Theme is now feature-complete and ready for an optional default-theme switch follow-up.
