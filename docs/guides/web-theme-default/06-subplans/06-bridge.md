...
**Route:** `/bridge`
**File:** `themes/default/routes/BridgeView.js`
**Status:** ⏭️ Skipped — `/bridge` has no nav entry point in the `default` UI (no sidebar / top-bar / footer link). Restyle deferred until the route is surfaced or the page is repurposed. Revisit in sub-plan 06.9 polish if needed.
...
> Restyles `default`'s BridgeView containers/inputs/buttons using R1–R7 tokens. Swap/signer logic unchanged. Closes sub-plan 06.6.
