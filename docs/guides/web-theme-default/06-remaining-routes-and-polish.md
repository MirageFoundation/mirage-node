...
**Goal:** Finish every route and every component that still renders in `oldreddit` style, perform a polish pass across the whole `default` theme, and verify it is ready to be the default.

**Depends on:** Plans 01–05.
**Unblocks:** making `default` the default theme (optional — can happen later).

**Status:** ✅ Done — sub-plans 06.1–06.5 and 06.7–06.11 ✅ done, 06.6 ⏭️ skipped. Split into sub-plans under [`06-subplans/`](./06-subplans/README.md).

---

## Why this plan expanded (2026-04-18 audit)

A full diff of `themes/default/**` vs `themes/oldreddit/**` revealed that a large set of routes and components were **copied from `oldreddit` with only `MobileHeader` mounted** and never actually restyled. They still use oldreddit tokens, spacing, and typography — which means every one of these screens currently violates R1, R2, R5, and R7 from [`RULES.md`](./RULES.md).
...
- [ ] Perform a vote, comment, follow, and block while on `default`.
...
- **Default theme switch risk** → do **not** flip `default` to index 0 in `THEME_MANIFESTS` until sub-plan 09 is merged and validated.

---

## Optional follow-up (separate PR after sub-plan 09)

- Move `defaultManifest` to index 0 in `THEME_MANIFESTS` to make it the default.
...
> [Sub-plan NN / short title] — restyles `<routes|components>` in `default` to use R1–R7 tokens and mobile-app visuals. Visual only. Closes sub-plan NN of Plan 06.
