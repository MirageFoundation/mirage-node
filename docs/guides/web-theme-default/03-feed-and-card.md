...
**Status:** ✅ Done — shipped in `default`. A manual cross-theme browser regression pass is still recommended.
...
- Rewrite `routes/MainView.js` inside `default` to use the new feed layout.
...
Beyond the original scope, the following refinements were applied during iteration and are shipped in the current `default` theme:
...
> Rewrites `default`’s feed, CardView, FilterBar, and VoteSection to match the mobile app’s post layout and action row, while keeping data hooks and prop shape unchanged. Post detail and profile follow in Plan 04.
