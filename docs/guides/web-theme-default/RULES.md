# default Theme — Style Rules

**Source of truth for styling decisions in the `default` web theme.** Every sub-plan and every future UI change MUST follow these rules. When the user gives color/divider/layout instructions, consult this file first.
...
| Token       | Dark      | Light     | Use                                                                                                                            |
| ----------- | --------- | --------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `tierAdmin` | `#EF4444` | `#DC2626` | Admin tier accent (admin tier label, admin chip, admin section accents). Use anywhere `getTierColor(level)` resolves to admin. |
...
Amber. Deliberately identical in both modes (like `link` / `focusBlue`).

| Token            | Dark / Light (shared)     | Use                                                     |
| ---------------- | ------------------------- | ------------------------------------------------------- |
| `warningBg`      | `rgba(245,158,11,0.08)`   | Warning card / banner / badge background.               |
| `warningBorder`  | `rgba(245,158,11,0.25)`   | Warning card / banner / badge border.                   |
| `warningHoverBg` | `rgba(245,158,11,0.15)`   | Warning hover (interactive warning pills only).         |
| `warningText`    | `#f59e0b`                 | Warning icon + label color.                             |

> `SettingsView::SecurityBanner`, `SettingsView::SeedWarningBanner`, `WelcomeView`,
> `CreatePostView` and `SubscriptionView::StatusBadge` still inline these as raw
> `rgba(245,158,11,…)` literals. Route them through the tokens when you next
...
2. If matched → update both dark and light values in `web/frontend/src/themes/default/tokens.js` so the pair stays consistent. Update this table too.
...
Before opening a PR for any default change, confirm:
...
The comment composer's textarea is the **canonical input focus style** for the entire theme. Every input / textarea / select / pill-trigger in `default` MUST follow it.
...
`focusBlue` is reserved for **active indicators** (selected tab underline, unread dot, active link). Using it as a focus ring on every input makes the whole theme feel "Reddit blue"; the default visual language is neutral / monochrome with blue as an accent, not a background.

### Applies to

All text inputs, textareas, selects, searchable selectors (`CommunitySelector`, renamed from `TopicSelector` in v1.39.0), pill toggles (`TagPill`, "Add content warning"), community pickers, comment composer, create-post composer, inline editors, settings inputs.
...
- `themes/default/ListFeedView.js` — sort / view / expand chevrons.
- `themes/default/components/Sidebar.js::ChevronIcon` — community expand chevron.
- `themes/default/routes/SettingsView.js::ChevronPill` — settings row chevron.
...
This keeps every form block (Create Post title, body, link URL, tag select, CommunitySelector, comment composer textarea, inbox search, etc.) at the **same typographic weight** regardless of which component renders it.
...
| 2026-09-01 | Added the R2 `warning*` token quartet (`warningBg` / `warningBorder` / `warningHoverBg` / `warningText`) so the amber that five components had inlined as raw `rgba(245,158,11,…)` has a home. Rebuilt `components/AccountStatusNotices.js` as hero cards matching the `MainView` app-download / NSFW consent language (icon tile + 0.78rem/600 title + 0.65rem body, 8px radius, R1 `bg` for the neutral card and the warning pair for the renewal card), replacing the 150px label/value grid that wrapped "Subscription renewal:" onto two lines. Quota notice gained a progress track and a reset countdown; the renewal countdown now reports minutes/hours instead of rounding every sub-day remainder up to "1 day". Both cards share one row order (header with a right-aligned headline value, then body, then affordance). The renewal action is an amber pill on SubscriptionView's StatusBadge geometry (999px, 0.3rem x 0.7rem, 0.65rem/600) rather than the shared `gradient` CTA — the brand gradient is purple and reads as a foreign element on an amber card. The warning icon tile uses `warningHoverBg`, since `warningBg` matches the card it sits on and leaves the tile invisible. `NoticeList` keeps a 1rem horizontal inset (0.85rem at 1000px, flush at 600px) to track `HomeFeedTitleBar` and `SectionBody`; going flush outdents the cards past every sibling section. |
| 2026-04-30 | Added R2 `tierAdmin` token pair (dark `#EF4444` / light `#DC2626`) under a new "Tiers" sub-section. Routes the admin tier accent (admin tier label in `ProfileView` / `SubscriptionView`) through tokens so light mode can diverge from the cross-theme `ADMIN_COLOR` constant in `logic/useSubscription.js`. Sub-plan 06.11.A. |
| 2026-09-02 | **Supersedes the `NoticeList` inset claim in the 2026-09-01 entry, which was wrong.** Cards in a feed column are full width with `margin: 4px 0` and carry their own `1rem` padding — the host supplies the gutter (`FeedHeroColumn` has none, `SectionBody` has `0 1rem`). Adding a card margin on top double-insets it in `SectionBody`, putting body text at 2rem while the heading above stays at 1rem. `AccountStatusNotices::NoticeCard` moved onto the `CardView` / `NsfwWelcomeHero` geometry accordingly. The quota notice reverted from the 2026-09-01 card-with-progress-track back to a plain `label: value` row copying `ProfileView::ProfileFieldRow` (150px label column, `0.55rem 1rem`), since it is ambient information that must sit on the same rhythm as the `Balance:` row beside it. Added `components/CreatorEarningsBanner.js` for claimable creator rewards on home / following, rebuilt on the retired `QuestHeroCard` geometry (24px glyph-only icon tile, `0.72rem/600` title over `0.6rem` subtitle, right-aligned amount pill at `0.55rem/700` / 999px on the `voteUp` + `voteUpBg` pair, full-width `followBtnBg` CTA in a `0 1rem 0.6rem` body) with every raw hex that card inlined routed through tokens per R213. Amounts use `formatMirageCompact` (2 decimals) like the `Balance:` field — the six-decimal `formatMirage` stays on the profile ledger rows where exact values matter. The body carries that card's `RewardRow` list verbatim (`panelAlt` fill, 10px radius, `0.55rem 0.8rem`, 28px `voteUp`-tinted icon, `0.62rem/600` uppercase label over the post title, right-aligned mono amount), one row per post that earned, capped at three with a `+N more` note. A claim card must show which posts produced the total: an unexplained number reads as arbitrary, and the prose version of that explanation belongs in `docs/FAQ.md`, not on the card. |
