...
**Route:** `/sign_out`
**File:** `web/frontend/src/themes/default/routes/SignOutView.js`
**Status:** ✅ Done
...
Rewrite `default`'s `SignOutView` as a short confirmation screen that reuses the auth-slice `AuthPageShell` primitives, so the sign-out flow feels intentional and on-brand.
...
- **Auth shell (reuse):** `themes/default/components/AuthPageShell.js` (from commit `ceef3d7`)
...
### In scope
- Rewrite `themes/default/routes/SignOutView.js`.
...
2. Read `themes/default/routes/LoginView.js` for `AuthPageShell` usage.
3. Copy auth-slice structure into `themes/default/routes/SignOutView.js`.
...
> Rewrites `default`'s `SignOutView` as a focused confirmation screen using the auth slice's `AuthPageShell` primitives. Visual only — sign-out handler and session logic unchanged. **Closes Plan 05.**
