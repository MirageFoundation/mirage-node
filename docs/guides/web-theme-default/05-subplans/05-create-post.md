...
**Route:** `/create_post`
**File:** `web/frontend/src/themes/default/routes/CreatePostView.js`
**Status:** ✅ Done
...
Rewrite `default`'s `CreatePostView` as a panel-styled composer that matches the mobile app's tone, with a sticky `MarkdownEditor` toolbar on desktop and media attach row using theme tokens.
...
### In scope
- Rewrite `themes/default/routes/CreatePostView.js`.
...
> Rewrites `default`'s `CreatePostView` as a panel-styled composer with mobile-app typography, sticky `MarkdownEditor` toolbar on desktop, and themed media attach row. Visual only — `useCreatePost` and media upload helpers unchanged.
