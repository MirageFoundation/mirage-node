...
- `burn_sequence` is per-chain, but records are keyed only by `burn_id` (sequence). With multiple destination chains, identical sequences can collide and overwrite/mis-associate burns/mints.
- Inbound `burn_id` is the external tx hash; a single Solana transaction can contain multiple burn instructions, which would share the same tx hash and collapse multiple burns into one attestation record.
