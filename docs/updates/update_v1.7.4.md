...
- **Removed soft-pinning** - posts no longer get boosted just for being from followed users in the home feed
...
### Stats: subscribers by tier

- Stats page now shows **subscriber breakdown by tier** (Tier 1, 2, 3) instead of just total
- Fixed indexer not updating `subscription_expiry` on subscription renewals
...
- Fixed inbox "Mark read" button following link instead of marking as read
- Fixed feed reloading unnecessarily on back navigation
...
- **Indexer**: fixed `subscription_expiry` not updating on `subscription_renewed` EndBlock events
- **Scripts**: `scripts/review_accounts_ai.py` - improved sockpuppet detection with better fingerprint weighting and vote analysis
- **Scripts**: `scripts/list_burns.py` - analyze token burns by category (fee_collector, subscriber relay, subscription fees, leftover reserves)
- **Docs**: `docs/tokenomics-burns.md` - documentation of all burn mechanisms
