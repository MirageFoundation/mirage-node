...
A one-time migration runs automatically on upgrade and copies all backend-owned rows from the old shared database into the new one. The entrypoint validates that both databases and the read-only role exist before any service starts, and refuses to boot if anything is missing. A new verification script (`scripts/verify_upgrade.py`) checks schema correctness, data migration completeness, read-only enforcement, and API health in one pass — run it after any upgrade to confirm everything landed cleanly.
