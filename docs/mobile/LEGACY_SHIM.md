...
- `GET /api/search_topics?q=...` returns `{"topics":[...]}` and preserves the old limit/offset behavior.
...
When all proof fields are absent, `X-Mirage-Visitor` is absent, and `address` is a valid Mirage address, ordinary feed and thread reads temporarily use that address for personalization. Anyone can spoof it. This path may alter vote, block, and tag presentation, but it never sets verified identity and is not used for writes, rewards, visitor attribution, curator reads, or admin authorization. Partial proofs still fail, and signed modern reads keep their normal verification.
...
- `legacy_mobile.thread_read_only`: rejected reply to a pre-v1.39 thread.
...
3. Delete `web/backend/legacy_mobile_wiring.py`. Restore strict protocol-1 post and explicit-period subscription handling, remove `legacy_thread_read_only` from `web/backend/error_utils.py`, and remove compatibility-only arguments/branches from `web/backend/pow.py` and `shared/canon.py`.
