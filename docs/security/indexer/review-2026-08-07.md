...
After remediation, create `docs/security/indexer/review-2026-08-07-retest.md` with a status row for every finding.

Minimum evidence:

- **H-1:** injected failure after each required write rolls the whole block back; checkpoint/hash stay unchanged; replay produces byte-for-byte-equivalent relevant rows; `--height` cannot mutate a non-empty DB unsafely.
- **H-2:** wrong chain ID and one mismatched overlap hash both stop startup before any overwrite; canonical recovery resumes normally.
- **H-3:** partial results trigger bounded retry; a later non-zero result is recorded only as failed and never reaches its handler.
- **H-4:** gov v1 multi-message proposal updates indexed state with REST disabled; transient proposal query failure retries the same block.
- **H-5:** redirects/private DNS/rebinding targets are blocked; oversized and slow responses cannot hold checkpoint progress beyond the declared budget.
- **M-1:** an unavailable `last_height + 1` exits non-zero unless an explicit partial-history mode is selected and exposed.
- **M-2:** failed balance queries leave existing rows unchanged.
- **M-3:** complete profile deletion/current-list reconciliation tests; partial source response fails closed.
- **M-4:** catch-up and gap-fill produce the documented difficulty/supply samples.
- **M-5:** migration import/read failure aborts startup; crash before marker leaves no partial database transaction.
- **M-6:** startup logs contain no credential substring.
- **M-7:** empty-title successful root posts and blocks before/after a param change project consistently.
- **M-8:** every vote transition and replay leaves `user_topic_stats.net_votes` equal to the sum of current canonical votes.
- **M-9:** injected profile/params/stats failures roll back the vote and checkpoint; no fallback weight is stored.
- **L-1:** URL query dimensions survive insert.
- **I-1:** a foreign edit/delete submitted directly to the chain succeeds there but leaves indexed content unchanged.
