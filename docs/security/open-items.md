...
**As of:** 2026-08-14, at the `v1.36.0` tag on `dev`. Pinned to the tag rather than a commit hash, which went stale the moment the release was amended.
...
- [backend full review 2026-08-14](2026-08-14/backend-review.md) — all severities, `web/backend/` only (1 Critical, 3 High, 5 Medium, 2 Low). **All dispositioned on 2026-08-14: ten fixed and shipped in `v1.36.0`, M-5 accepted as risk.** **Authoritative for the backend items below**, and it supersedes the 2026-08-07 backend retest as the current statement of backend posture
...
**Two Critical and eight High**, from four separate full reviews on 2026-08-14. **Three remain unfixed**, all from the [blockchain review](2026-08-14/blockchain-review.md). The two indexer High, the backend Critical plus three backend High, and the frontend High all shipped in `v1.36.0`. The shipped rows are kept in the table so the history stays readable.
...
The backend Critical, all three backend High, the two indexer High and the frontend High all shipped in `v1.36.0`; none needs further action.
...
upgrade. Until a token is issued *and* enhanced security is enabled in the Expo
dashboard, anyone holding a copy of the `push_tokens` table can send pushes to users.
This is the one open item from this review.
