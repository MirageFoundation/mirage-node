...
**The corrupt-profile-row exercise is done, and automated rather than performed once.** The gate asked for a damaged profile row to be read against a local indexer with the outcome recorded. It is now `indexer_hardening.corrupt_profile_degrades`, which writes a row straight into the indexer DB — bypassing the indexer, so it holds shapes the indexer could never produce: a NULL username where the reader coerces, a negative level and expiry no handler can emit, and an owner that is not a valid address — then reads it back through `/api/get_profile` and cleans up. **The recorded answer is that it degrades, and precisely how matters.** The observed response is a 200 in which the NULL username reads as empty — `_db_get_profile_scalars` guards for `None` — while `level`, `created_at` and `subscription_expiry` are returned to the client as `-1`, verbatim. So the row is *not* sanitised: unreadable fields coerce, but type-valid nonsense passes straight through. That is still a degradation and not a halt, which is what the gate asked: the request returns rather than hanging, no worker dies, and the process serves the next request normally, which is the half that separates the two. It is not a privilege question either, since tier checks compare upward and the authoritative level lives on chain, and nothing about the row can reach consensus because it exists only in this node's index and could only have been written by something that already had database access. The test asserts the coercion rather than merely accepting a 200, so a future change that started serving a NULL username as `null` would fail here. Verified against the live local stack.
