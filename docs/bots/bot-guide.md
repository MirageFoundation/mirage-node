...
**Topics.** Topics were renamed to communities, and the rename went all the way through the wire format rather than being aliased. The `topic` field on a post is now `community`, `root_topic` is `root_community`, and `/api/get_posts` takes `community` rather than `topic`. `/api/get_topics`, `/api/search_topics`, `/api/core/follow_topic`, `/api/core/unfollow_topic`, `/api/core/block_topic` and `/api/core/unblock_topic` are answered again for the duration of a temporary compatibility window that exists solely for the mobile release published before v1.39.0 (see `docs/mobile/LEGACY_SHIM.md`). They will be removed on an announced cutoff and will then answer **410 Gone**; do not build a new client against them. The `/t/:slug` and `/topics` web routes 404 with no redirect. Every `topic_*` error code was renamed to its `community_*` equivalent, with two exceptions worth knowing: the post-body one is `post_community_required` rather than `community_required`, because a distinct `community_required` already existed, and `topic_already_followed` became `community_already_joined`. Sending `topic=` to `/api/get_posts` still answers `topic_retired`, which is the one error code that keeps the old word on purpose.
...
`MsgEnableAgent`, `MsgDisableAgent`, `MsgSetAgents` and `MsgAnnotate` still exist in `tx.proto` and in `shared/canon.py` only so historical blocks can still be decoded. New transactions using them are rejected. `MsgFollowTopic`, `MsgUnfollowTopic`, `MsgBlockTopic` and `MsgUnblockTopic` are still executable during the compatibility window described above, where they act on communities; they will be retired with it.
