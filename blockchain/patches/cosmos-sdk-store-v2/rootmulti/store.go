...
func (rs *Store) PruneStores(pruningHeight int64) (err error) {
	...
	// versions above and bumps the s/earliest pointer, but never deletes the
	// rootmulti commit-info records (s/<version>) that flushCommitInfo writes on
	// every Commit. They therefore accumulate without bound (one per block,
	// fleet-wide) and become the dominant on-disk growth (the s/earliest pointer
	// only narrows the queryable range; it frees no disk). Delete the now-stale
	// ones too. This is consensus-safe: the app hash is computed from the CURRENT
...
//
// The metadata DB shares the "s/" keyspace with "s/latest", "s/earliest" and the
// IAVL substore data ("s/_/...", "s/k:<name>/..."). Commit-info keys are "s/"
// followed by decimal digits (0x30-0x39), which sort strictly before 'e'
// (0x65, earliest), 'l' (0x6c, latest), '_' (0x5f) and 'k' (0x6b), so the
// half-open range ["s/0", "s/:") covers exactly the commit-info keys and never
// walks the (large) IAVL node data. The suffix is still parsed defensively and
// any non-integer key is skipped.
