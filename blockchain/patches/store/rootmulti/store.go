...
func (rs *Store) PruneStores(pruningHeight int64) (err error) {
	...
	// Mirage patch (NOT upstream): upstream PruneStores prunes only the IAVL
	// versions above and never touches the rootmulti commit-info records
	// (s/<version>), which flushCommitInfo writes on every Commit. They
	// therefore accumulate without bound (one per block, fleet-wide) and become
	// the dominant on-disk growth. Delete the now-stale ones too. This is
	// consensus-safe: the app hash is computed from the CURRENT commit-info, and
	// commit-info below the pruning height only describes already-pruned,
	// unqueryable state. Work is capped per pass so a large historical backlog
	// drains across many prune passes instead of stalling a block; a failure
	// here is logged but never aborts the prune pass.
...
// PruneStores pass may delete, keeping per-block latency bounded while a large
// historical backlog drains over successive passes.
const commitInfoPruneBatch = 20000

// pruneCommitInfo deletes rootmulti commit-info records (key "s/<version>") for
// versions strictly below pruningHeight. See the call site in PruneStores for
// why this is safe and necessary.
//
// The metadata DB shares the "s/" keyspace with "s/latest" and the IAVL substore
// data ("s/_/...", "s/k:<name>/..."). Commit-info keys are "s/" followed by
// decimal digits (0x30-0x39), which sort strictly before '_' (0x5f), 'k' (0x6b)
// and 'l' (0x6c), so the half-open range ["s/0", "s/:") covers exactly the
// commit-info keys and never walks the (large) IAVL node data. The suffix is
// still parsed defensively and any non-integer key is skipped.
func (rs *Store) pruneCommitInfo(pruningHeight int64) error {
	...
	// Collect the keys to delete, then close the iterator BEFORE writing. Some
	// backends (e.g. the cosmos-db MemDB) hold a read lock for the iterator's
	// lifetime via a producer goroutine; writing the delete batch while that
	// iterator is still open deadlocks. Collect-then-close-then-write is correct
	// for every backend. The cap bounds the slice to commitInfoPruneBatch keys.
	...
	for ; iter.Valid() && len(keys) < commitInfoPruneBatch; iter.Next() {
		...
		if perr != nil {
			// Not a commit-info record (defensive; should not occur in this range).
			continue
		}
		if version >= pruningHeight {
			continue
		}
		// Copy: the iterator may reuse/invalidate the key slice on Next().
	...
	defer batch.Close()
	for _, k := range keys {
		if derr := batch.Delete(k); derr != nil {
			return derr
		}
	}
	...
	rs.logger.Debug("pruned commit-info records", "deleted", len(keys), "prune_to", pruningHeight)
	return nil
}
