...
def init_backend_schema() -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            # Gunicorn workers each run this. Without a lock, concurrent
            # CREATE INDEX IF NOT EXISTS races on pg_class and UniqueViolation
            # kills the worker (backend FATAL, 503 on every route).
            cur.execute("SELECT pg_advisory_lock(%s)", (872514093,))
