...
2. **Import** that manifest in **`src/themes/manifests.js`** and **append** it to **`THEME_MANIFESTS`**.  
   - **Order matters:** **`THEME_MANIFESTS[0]`** is **`DEFAULT_THEME_ID`** — used when storage is missing, invalid, or unknown.
...
`AgentsView`, `BlocksView`, `BridgeView`, `ChangeUsernameView`, `CreateAccountView`, `CreatePostView`, `DiscoverView`, `FollowsView`, `InboxView`, `LoginView`, `MainView`, `NetworkView`, `NotFoundView`, `ProfileView`, `ReferralsView`, `ReportsView`, `SearchResultsView`, `SettingsView`, `SignOutView`, `StatsView`, `SubscriptionView`, `ViewPostView`, `WelcomeView`.
