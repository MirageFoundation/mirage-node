...
import MobileBottomNav, { DEFAULT_BOTTOM_NAV_HEIGHT } from './components/MobileBottomNav';
import Storage from '../../utils/Storage';

/**
 * Root shell for the `default` theme.
 *
 *  Desktop (> 1000px):
 *      ┌──────────────────────────────────────────────────────────┐
 *      │ TopBar (sticky)                                          │
...
const SIDEBAR_HIDDEN_KEY = 'default_sidebar_hidden';
...
const Main = styled.main.attrs(({ $hidden }) => ({
    ...
    @media (max-width: 600px) {
        ...
        padding: 0 0.75rem calc(${DEFAULT_BOTTOM_NAV_HEIGHT}px + 1rem + env(safe-area-inset-bottom, 0px));
    }
`;
...
export default function DefaultShell({ children, state }) {
